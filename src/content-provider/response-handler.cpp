...
void ConvertionResponseHandler::sendRequest(const std::string& city_name) {
    response_ = cpr::Get(cpr::Url{CONVERTION_BASE_URL},
...
std::pair<double, double> ConvertionResponseHandler::parseResponse() {
    ...
    return {response_data_array[0]["latitude"].GetDouble(),
...
void ForecastResponseHandler::sendRequest(const Forecast& forecast,
                                          const Config& config) {
    response_ = cpr::Get(
        ...
        cpr::Parameters{
            ...
            {"daily",
             ...
             "sunset,precipitation_sum,windspeed_10m_max"},
...
Forecast ForecastResponseHandler::parseResponse() {
    auto response_body =
        json_response_.Parse(response_.text.c_str()).GetObject();
    auto daily_obj = response_body["daily"].GetObject();
    ...
    auto windspeed_10m_array = daily_obj["windspeed_10m_max"].GetArray();
    ...
    for (int i = 0; i < forecast_days; ++i) {
        data_array[i].configure(
            date_array[i].GetString(), sunrise_time_array[i].GetString(),
            sunset_time_array[i], weather_code_array[i],
            temperature_2m_max_array[i],
            apparent_temperature_max_array[i],
            precipitation_sum_array[i],
            windspeed_10m_array[i]);
    }
...
void TimezoneConvertResponseHandler::sendRequest(const std::string& city_name) {
    response_ = cpr::Get(cpr::Url{TIMEZONE_BASE_URL},
...
bool TimezoneConvertResponseHandler::checkSuccess() { return false; }
